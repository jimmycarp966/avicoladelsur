-- Soporte de tarifa por turno trabajado para puestos con tipo_calculo='turno'
-- Regla: turno completo = 1 unidad, medio turno = 0.5 unidad.

ALTER TABLE public.rrhh_liquidacion_reglas_puesto
ADD COLUMN IF NOT EXISTS tarifa_turno_trabajado DECIMAL(12,2) NOT NULL DEFAULT 0;
...
CREATE OR REPLACE FUNCTION public.fn_rrhh_resolver_regla_puesto(
  ...
  p_mes integer,
  p_anio integer,
  p_puesto_override text DEFAULT NULL::text
)
RETURNS TABLE(
  puesto text,
  ...
  tarifa_turno_trabajado numeric,
  ...
  SELECT
    ...
    CASE
      WHEN COALESCE(v_regla.tipo_calculo, 'hora') = 'turno' THEN
        CASE
          WHEN lower(COALESCE(a.turno, '')) IN ('medio_turno_manana', 'medio_turno_tarde', 'manana', 'mañana', 'tarde') THEN 0.5
          ELSE 1
        END
