-- RRHH: Legajo unificado, descansos mensuales de sucursal y normalizaci�n de sucursales
-- Fecha: 2026-03-04
...
-- =====================================================
-- 4) Normalizaci�n de sucursales: Col�n -> Horqueta, Sucursal Central -> Casa Central
-- =====================================================
DO $do$
DECLARE
  v_colon uuid;
...
BEGIN
  ...
  WHERE translate(lower(nombre), '�����', 'aeiou') LIKE '%sucursal colon%'
  LIMIT 1;

  SELECT id INTO v_horqueta
  FROM public.sucursales
  WHERE translate(lower(nombre), '�����', 'aeiou') LIKE '%sucursal horqueta%'
  LIMIT 1;

  SELECT id INTO v_sucursal_central
  FROM public.sucursales
  WHERE translate(lower(nombre), '�����', 'aeiou') LIKE '%sucursal central%'
  LIMIT 1;

  SELECT id INTO v_casa_central
  FROM public.sucursales
  WHERE translate(lower(nombre), '�����', 'aeiou') LIKE '%casa central%'
  LIMIT 1;
