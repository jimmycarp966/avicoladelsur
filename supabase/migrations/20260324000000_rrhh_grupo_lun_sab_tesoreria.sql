...
ALTER TABLE public.rrhh_liquidacion_reglas_periodo
  ADD COLUMN IF NOT EXISTS dias_base_lun_sab integer NOT NULL DEFAULT 26 CHECK (dias_base_lun_sab > 0);

ALTER TABLE public.rrhh_liquidacion_reglas_puesto
  DROP CONSTRAINT IF EXISTS rrhh_liquidacion_reglas_puesto_grupo_base_dias_check;

ALTER TABLE public.rrhh_liquidacion_reglas_puesto
  ADD CONSTRAINT rrhh_liquidacion_reglas_puesto_grupo_base_dias_check
  CHECK (grupo_base_dias IN ('galpon', 'sucursales', 'rrhh', 'lun_sab'));

DROP FUNCTION IF EXISTS public.fn_rrhh_preparar_liquidacion_mensual(uuid, integer, integer, uuid);
DROP FUNCTION IF EXISTS public.fn_rrhh_recalcular_liquidacion(uuid, uuid);
...
UPDATE public.rrhh_liquidacion_reglas_puesto
SET
  grupo_base_dias = 'lun_sab',
  updated_at = now()
WHERE lower(trim(puesto_codigo)) = 'tesoreria';
