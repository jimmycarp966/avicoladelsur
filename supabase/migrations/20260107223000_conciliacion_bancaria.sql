...
-- Date: 2026-01-07
-- Description: Create tables for bank reconciliation and update clients table.
...
-- 2. Tabla de Cuentas Bancarias (Requisito para el selector)
...
CREATE TABLE IF NOT EXISTS movimientos_bancarios (
    ...
    metadata JSONB DEFAULT '{}', -- Para guardar info extra del parser/IA
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS pagos_esperados (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    pedido_id UUID, -- Referencia débil a pedidos si no queremos FK estricta aún, o REFERENCES pedidos(id)
    cliente_id UUID, -- REFERENCES clientes(id)
    monto_esperado DECIMAL(12,2) NOT NULL,
    fecha_esperada DATE,
    referencia TEXT,
    dni_cuit TEXT,
    estado VARCHAR(20) DEFAULT 'pendiente', -- pendiente, conciliado, cancelado
    origen TEXT DEFAULT 'pedido', -- pedido, factura, manual
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS conciliaciones (
    ...
    tipo_match VARCHAR(20), -- exacto, automatico, manual, ia
    confianza_score DECIMAL(5,2), -- 0.00 a 100.00
...
-- Helper function to check role (assuming public.usuarios has roles and matches auth.uid)
-- OR just simpler policies for now.
-- Policy: Allow read/write to 'tesorero' and 'admin'
