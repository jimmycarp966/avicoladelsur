...
DROP POLICY IF EXISTS "Usuarios autenticados pueden leer mensajes" ON bot_messages;
...
DROP POLICY IF EXISTS "Solo service role puede insertar mensajes" ON bot_messages;
...
DROP POLICY IF EXISTS "No se pueden actualizar mensajes" ON bot_messages;
CREATE POLICY "No se pueden actualizar mensajes"
ON bot_messages FOR UPDATE
TO authenticated
USING (false);

DROP POLICY IF EXISTS "No se pueden eliminar mensajes" ON bot_messages;
