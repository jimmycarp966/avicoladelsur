-- ===========================================
-- MIGRACIÓN: Fix Conversión Mayorista Pesable
-- Fecha: 12/01/2026
-- Objetivo: Corregir bug donde items mayoristas pesables ignoraban el peso_final
--           y aplicaban multiplicación x bulto incorrectamente.
-- ===========================================
...
BEGIN
    ...
    INSERT INTO entregas (
        pedido_id, cliente_id, presupuesto_id, subtotal, recargo, total,
        direccion, coordenadas, orden_entrega, estado_entrega, estado_pago,
        referencia_pago, instruccion_repartidor, observaciones
    ) VALUES (
        v_pedido_id, v_presupuesto.cliente_id, p_presupuesto_id,
        v_total_items, COALESCE(v_presupuesto.recargo_total, 0), v_total_con_recargo,
        v_cliente.direccion, v_cliente.coordenadas, v_orden_entrega, 'pendiente', 'pendiente',
        v_referencia_pago, v_instruccion_repartidor, v_presupuesto.observaciones
    ...
    LOOP
        ...
        -- FIX: PRIORIDAD A PESABLE
        v_cantidad_a_consumir := CASE
            -- 1. Si es pesable, SIEMPRE usar peso_final (o solicitado si falta)
            WHEN v_item.pesable THEN 
                COALESCE(v_item.peso_final, v_item.cantidad_solicitada)
            -- 2. Si es Mayorista (y no pesable), usar cantidad cajas
        ...
        IF v_item_lista_tipo IN ('mayorista', 'distribuidor')
           ...
           AND NOT v_item.pesable THEN -- FIX: NO multiplicar si es pesable (ya está en Kg)
        ...
        IF v_item.lote_reservado_id IS NOT NULL THEN
            ...
            INSERT INTO movimientos_stock (
                ...
                v_item.lote_reservado_id, 'salida', v_cantidad_stock,
                ...
                CASE WHEN v_cantidad_stock != v_cantidad_a_consumir
                     ...
                     ELSE '' END,
                p_user_id, v_pedido_id
            );
        END IF;
    END LOOP;

    -- Finalizar presupuesto
    UPDATE stock_reservations SET estado = 'consumida' WHERE presupuesto_id = p_presupuesto_id;
    UPDATE presupuestos SET estado = 'facturado', pedido_convertido_id = v_pedido_id, total_final = v_total_con_recargo, updated_at = NOW() WHERE id = p_presupuesto_id;
    UPDATE pedidos SET subtotal = subtotal + v_total_items, total = total + v_total_con_recargo, total_final = total_final + v_total_con_recargo, cantidad_entregas = cantidad_entregas + 1, updated_at = NOW() WHERE id = v_pedido_id;

    -- Cta Cte
    IF p_caja_id IS NULL THEN
        v_cuenta_id := fn_asegurar_cuenta_corriente(v_presupuesto.cliente_id);
        UPDATE cuentas_corrientes SET saldo = saldo + v_total_con_recargo, updated_at = NOW() WHERE id = v_cuenta_id;
        INSERT INTO cuentas_movimientos (cuenta_corriente_id, tipo, monto, descripcion, origen_tipo, origen_id)
        VALUES (v_cuenta_id, 'cargo', v_total_con_recargo, 'Entrega del pedido ' || v_numero_pedido || ' - Ref: ' || v_referencia_pago, 'entrega', v_entrega_id);
        UPDATE clientes SET bloqueado_por_deuda = true WHERE id = v_presupuesto.cliente_id;
    END IF;

    -- Caja
    IF p_caja_id IS NOT NULL THEN
        INSERT INTO tesoreria_movimientos (caja_id, tipo, monto, descripcion, origen_tipo, origen_id, user_id)
        VALUES (p_caja_id, 'ingreso', v_total_con_recargo, 'Cobro por entrega del pedido ' || v_numero_pedido, 'entrega', v_entrega_id, p_user_id);
        UPDATE tesoreria_cajas SET saldo_actual = saldo_actual + v_total_con_recargo, updated_at = NOW() WHERE id = p_caja_id;
        UPDATE entregas SET estado_pago = 'pagado', monto_cobrado = v_total_con_recargo WHERE id = v_entrega_id;
    END IF;

    RETURN jsonb_build_object(
        'success', true, 'pedido_id', v_pedido_id, 'numero_pedido', v_numero_pedido, 'entrega_id', v_entrega_id, 'total', v_total_con_recargo,
