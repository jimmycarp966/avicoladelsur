...
import { actualizarListaPrecioAction } from '@/actions/listas-precios.actions'

interface ListaPrecioEditFormProps {
  lista: {
    id: string
    codigo: string
    nombre: string
...
export function ListaPrecioEditForm({ lista }: ListaPrecioEditFormProps) {
  ...
  const onSubmit = async (data: ListaPrecioInput) => {
    try {
      ...
      const result = await actualizarListaPrecioAction(lista.id, formData)
      ...
      showToast('error', error.message || 'Error al actualizar lista de precios')
    } finally {
      setIsLoading(false)
    }
  }
