...
async def run_test():
    ...
    try:
        ...
        # -> Input valid username and password for admin role and click login
        ...
        # -> Logout admin and navigate back to login page to test next role vendedor
        frame = context.pages[-1]
        # Click on Reportes or user menu to find logout option
        elem = frame.locator('xpath=html/body/div[2]/div/div/nav/ul/li/ul/li[10]/div/a').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        frame = context.pages[-1]
        # Click logout button or link if found
        ...
        await expect(frame.locator('text=Reporte de Reparto y Logística').first).to_be_visible(timeout=30000)
        await expect(frame.locator('text=Análisis de eficiencia, entregas y rendimiento de rutas').first).to_be_visible(timeout=30000)
