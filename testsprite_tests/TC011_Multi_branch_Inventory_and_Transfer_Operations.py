...
async def run_test():
    ...
    try:
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Inter-branch transfer completed successfully')).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError('Test case failed: The test plan execution failed to verify inventory segregation by branch, inter-branch stock transfer validations, and low stock alerts per branch.')
