...
async def run_test():
    ...
    try:
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=GPS monitor is perfectly synchronized with real-time delivery updates').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError('Test case failed: GPS monitor did not update delivery locations every 5 seconds with real-time route polylines or failed to trigger alert notifications for route deviations or jumps as required by the test plan.')
