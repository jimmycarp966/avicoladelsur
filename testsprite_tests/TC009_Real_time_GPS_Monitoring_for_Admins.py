...
async def run_test():
    ...
    try:
        ...
        # -> Click on GPS monitor dashboard link or button to open it
        frame = context.pages[-1]
        # Click Iniciar Sesión button to login
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Live GPS Tracking Unavailable').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test failed: The GPS monitor did not show live locations, polylines, or alerts as required by the test plan.")
