...
async def run_test():
    ...
    try:
        ...
        # -> Retry login with the same admin credentials to confirm if issue persists
        frame = context.pages[-1]
        ...
        # -> Navigate to 'Ventas' tab to perform order conversion workflow
        frame = context.pages[-1]
        # Click on 'Ventas' tab to start order conversion workflow
        elem = frame.locator('xpath=html/body/div[2]/div/div/nav/ul/li/ul/li[4]/div/a').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Click 'Nuevo Presupuesto' to create a new budget for order conversion testing
        frame = context.pages[-1]
        ...
        # -> Click client search button to open client selection dropdown or autocomplete
        frame = context.pages[-1]
        # Click client search button to open client selection dropdown or autocomplete
        ...
        # -> Select a client from the dropdown list to continue filling the budget form
        frame = context.pages[-1]
        ...
        # -> Select 'Tipo de Venta' as 'Reparto (entrega a domicilio)' and select a delivery zone
        frame = context.pages[-1]
        # Select 'Reparto (entrega a domicilio)' radio button for Tipo de Venta
        elem = frame.locator('xpath=html/body/div[2]/div[3]/main/div/div/form/div/div[2]/div[2]/div/label/input').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        frame = context.pages[-1]
        # Open Zona de Entrega dropdown to select a delivery zone
        elem = frame.locator('xpath=html/body/div[2]/div[3]/main/div/div/form/div/div[2]/div[3]/div/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Select a delivery zone from the dropdown to continue filling the budget form
        frame = context.pages[-1]
        ...
        # -> Open 'Lista de Precios (Global)' dropdown to select a price list
        frame = context.pages[-1]
        # Open 'Lista de Precios (Global)' dropdown
        elem = frame.locator('xpath=html/body/div[2]/div[3]/main/div/div/form/div/div[2]/div[4]/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Select 'MAYORISTA - Lista Mayorista (20% margen)' price list to continue filling the budget form
        frame = context.pages[-1]
        ...
        frame = context.pages[-1]
        # Select product '149 - ALAS KG.' from the product dropdown
        elem = frame.locator('xpath=html/body/div[3]/div/div/div[2]/div').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Click 'Agregar Producto' to add the product to the budget, then click 'Crear Presupuesto' to submit the form and create the budget
        frame = context.pages[-1]
        # Click 'Agregar Producto' to add product to budget
        ...
        try:
            ...
            raise AssertionError("Test case failed: Critical workflows using Server Actions with atomic database transactions did not complete successfully. Operations may have partially committed or rolled back improperly, violating data consistency.")
