...
async def run_test():
    ...
    try:
        ...
        # -> Click the 'Iniciar Sesión' button to attempt login again
        frame = context.pages[-1]
        # Click Iniciar Sesión button to login
        elem = frame.locator('xpath=html/body/div[2]/div[3]/div/div[2]/form/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        ...
        # -> Navigate to the WhatsApp chatbot interface or section to send a natural language order message
        frame = context.pages[-1]
        # Click on 'IA' menu to access AI/Chatbot features
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Order processed with zero items').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test failed: The WhatsApp chatbot did not correctly interpret the natural language order, validate stock using FIFO, or confirm the order as expected.")
