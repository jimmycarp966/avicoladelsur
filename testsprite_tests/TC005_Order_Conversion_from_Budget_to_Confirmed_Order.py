...
async def run_test():
    ...
    try:
        ...
        # -> Navigate to budgets section to create a new budget
        frame = context.pages[-1]
        # Click on the button to proceed after login (if applicable)
        elem = frame.locator('xpath=html/body/div[2]/div[3]/div/div[2]/form/div[2]/div/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        frame = context.pages[-1]
        # Click on 'Presupuestos' (Budgets) menu item to go to budgets section
        elem = frame.locator('xpath=html/body/div[2]/div[3]/div/div[2]/form/div[2]/div/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        ...
        await expect(frame.locator('text=No hay clientes en riesgo. Todos tus clientes están comprando regularmente.').first).to_be_visible(timeout=30000)
