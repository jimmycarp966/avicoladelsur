...
async def run_test():
    ...
    try:
        ...
        # -> Click on 'Navegación GPS' to start Spanish voice navigation and verify step-by-step guidance
        frame = context.pages[-1]
        # Click on Navegación GPS to start voice navigation
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Voice Navigation Activated Successfully').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test failed: The PWA did not activate Spanish voice navigation as required by the test plan.")
