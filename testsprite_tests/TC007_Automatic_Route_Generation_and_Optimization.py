...
async def run_test():
    ...
    try:
        ...
        # -> Navigate to delivery orders creation page or section
        frame = context.pages[-1]
        # Click button to proceed after login if available
        elem = frame.locator('xpath=html/body/div[2]/div[3]/div/div[2]/form/div[2]/div/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Click on 'Reparto' link in the left navigation menu to access delivery orders section
        frame = context.pages[-1]
        # Click 'Reparto' link in the left navigation menu to go to delivery orders section
        ...
        # -> Select vehicle, delivery person, date, shift, and zone to enable available orders selection
        frame = context.pages[-1]
        # Open vehicle selection dropdown
        elem = frame.locator('xpath=html/body/div[2]/div[3]/main/div/div/form/div/div[2]/div/div/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Select Repartidor (delivery person) from dropdown using semantic selectors
        frame = context.pages[-1]
        # Open 'Repartidor' dropdown using semantic selector
        elem = frame.locator('xpath=html/body/div[3]/div').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Select first available delivery person from dropdown
        frame = context.pages[-1]
        # Select first available delivery person option from dropdown
        elem = frame.locator('xpath=html/body/div[2]/div/div/nav/ul/li/ul/li[2]/div/a').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Click on 'Reparto' link in the left navigation menu to return to delivery routes section
        frame = context.pages[-1]
        # Click 'Reparto' link in the left navigation menu to go to delivery routes section
        elem = frame.locator('xpath=html/body/div[2]/div/div/nav/ul/li/ul/li[5]/div/a').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        ...
        try:
            ...
            raise AssertionError("Test failed: Delivery routes were not generated automatically using Google Directions API with fallback logic and vehicle capacity constraints as per the test plan.")
